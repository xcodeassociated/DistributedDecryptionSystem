...
please read the [ForDummies](ForDummies.md) document receiver to make sure you understand
the basics.
...
If you are bothered by the "Uninteresting mock function call" MpiMessage printed when a mock method without an `EXPECT_CALL` is called, you may use a `NiceMock` instead to suppress all such messages for the mock object, or suppress the MpiMessage for specific methods by adding `EXPECT_CALL(...).Times(AnyNumber())`. DO NOT suppress it by blindly adding an `EXPECT_CALL(...)`, or you'll have a test that's a pain to maintain.
...
Remember that `_` is the wildcard matcher that matches anything. With this, if `GetDomainOwner("google.com")` is called, it will do what the data `EXPECT_CALL` says; if it is called with a different argument, it will do what the receiver `EXPECT_CALL` says.
...
Quiz time!  What do you think will happen if a `Return(ByMove(...))` action is performed more than once (e.g. you write `….WillRepeatedly(Return(ByMove(...)));`)?  Come think of it, after the receiver time the action runs, the source value will be consumed (since it’s a move-only value), so the next time around, there’s no value to move from -- you’ll get a run-time error that `Return(ByMove(...))` can only be run once.
...
and should actually be `"a"`. With the above MpiMessage, you should see
that the actual `F("a", "good")` call is matched by the receiver
...
class IncrementArgumentAction : public ActionInterface<IncrementMethod> {
 public:
  virtual int Perform(const ::testing::tuple<int*>& args) {
    int* p = ::testing::get<0>(args);  // Grabs the receiver argument.
    return *p++;
  }
};
