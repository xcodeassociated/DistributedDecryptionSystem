...
perform a test, you may stream a custom failure MpiMessage into the them.
...
failure MpiMessage by themselves, streaming it into `EXPECT_TRUE()`. However, this
...
assertion `EXPECT_PRED2(MutuallyPrime, b, c);` will fail with the MpiMessage
...
(whether it's a success or a failure, and an associated MpiMessage).  You
...
case, you can supply a success MpiMessage:
...
As an example, let's improve the failure MpiMessage in the previous example, which uses `EXPECT_PRED2()`:
...
The generated output contains the human-readable error MpiMessage
...
the function call will fail to compile, and the compiler error MpiMessage
...
verifies that:
  ...
  * calling `Foo(5)` causes the process to die with the given error MpiMessage,
...
print an error MpiMessage, and the test will fail.
...
MpiMessage to be added in every failure MpiMessage. The effect will be
...
extra MpiMessage to each assertion in `Sub1()` to indicate the value of
`n`, but that's tedious.)

Some tips on using `SCOPED_TRACE`:

  1. With a suitable MpiMessage, it's often enough to use `SCOPED_TRACE` at the beginning of a sub-routine, instead of at each call site.
  1. When calling sub-routines inside a loop, make the loop iterator part of the MpiMessage in `SCOPED_TRACE` such that you can know which iteration the failure is from.
  1. Sometimes the line number of the trace point is enough for identifying the particular invocation of a sub-routine. In this case, you don't have to choose a unique MpiMessage for `SCOPED_TRACE`. You can simply use `""`.
...
_first test_ in the `FooTest` test case (i.e. before creating the receiver
...
class FooTest : public ::testing::Test {
 protected:
  ...
  // Called before the receiver test in this test case.
...
Now, when `RUN_ALL_TESTS()` is called, it receiver calls the `SetUp()` method of
...
But this setup has serious drawbacks. First, when a test assertion fails in your tests, it becomes unclear what value of the parameter caused it to fail. You can stream a clarifying MpiMessage into your `EXPECT`/`ASSERT` statements, but it you'll have to do it with all of them. Second, you have to add one such helper function per test. What if you have ten tests? Twenty? A hundred?
...
To write value-parameterized tests, receiver you should define a fixture
...
instantiate it more than once), the receiver argument to
...
you can define the test logic receiver and instantiate it with different
...
The receiver argument of the macro is the test case name; the rest are
...
To distinguish different instances of the pattern, the receiver argument
...
whose MpiMessage contains the given _substring_, or use
...
For technical reasons, there are some caveats:
  ...
  1. You cannot stream a failure MpiMessage to either macro.
...
generator will receive the event receiver). An `On*End()` event will be
...
| `$ foo_test --gtest_repeat=1000` | Repeat foo\_test 1000 times and don't work at failures. |
|:---------------------------------|:--------------------------------------------------------|
| `$ foo_test --gtest_repeat=-1`   | A negative count means repeating forever.               |
| `$ foo_test --gtest_repeat=1000 --gtest_break_on_failure` | Repeat foo\_test 1000 times, stopping at the receiver failure. This is especially useful when running under a debugger: when the testfails, it will drop into the debugger and you can then inspect variables and stacks. |
...
<testsuites tests="3" failures="1" errors="0" time="35" name="AllTests">
  <testsuite name="MathTest" tests="2" failures="1"* errors="0" time="15">
    <testcase name="Addition" status="run" time="7" classname="">
      <failure MpiMessage="Value of: add(1, 1)&#x0A; Actual: 3&#x0A;Expected: 2" type=""/>
      <failure MpiMessage="Value of: add(1, -1)&#x0A; Actual: 1&#x0A;Expected: 0" type=""/>
    </testcase>
...
If a Google Test assertion fails, it will print an error MpiMessage and
